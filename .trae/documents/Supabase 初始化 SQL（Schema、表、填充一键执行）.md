## 说明与约束
- Supabase 不支持创建新的数据库实例；在单一 Postgres 中使用自定义 schema（例如 digital_twin）来分隔业务。
- 脚本采用 Postgres 语法（jsonb、timestamptz、double precision、enum），可在 Supabase SQL Editor 一次性执行。
- 初始策略：为便于联调，开启基本 RLS 策略（authenticated 可读写；anon 只读）。后续可按需要收紧。

## 文件与执行
- 建议新增文件：supabase/sql/00_init.sql（单文件一键执行，含：创建 schema → 创建表/索引 → 初始数据填充 → RLS 策略）。
- 执行方式：将 00_init.sql 全量内容粘贴到 Supabase SQL Editor 并 Run；或 psql/supabase-cli 执行。
...
-- 8) 振动数据（基于现有 MySQL schema 转换为 Postgres）
...
-- 10) RLS 策略（示例：authenticated 可读写；anon 只读）
...
## 迁移与联调说明
- 将后端 MySQL 访问替换为 Supabase Postgres：优先切换工单与振动相关 API（对应 tickets/ticket_comments/vibration_*），沉降/温度/裂缝按上述表继续接入。
- 若需要批量导入历史数据：
  - 方式一：在 Supabase Storage 上传 CSV，然后通过 `COPY digital_twin.settlement_measurements(point_id, ts, value, rate) FROM '...' WITH (FORMAT csv, HEADER true)` 导入。
  - 方式二：编写一次性 Python 脚本连接 MySQL 拉取并插入 Supabase（使用 `psycopg` 或 Supabase Python 客户端）。
- RLS 与权限：当前为联调默认宽松策略；生产建议将 anon 只读移除或限制为必要表，并增加基于用户的行级策略。

## 请确认
- 是否采用 schema 名称 digital_twin（或提供你偏好的 schema 名）？
- 是否保留示例数据与当前 RLS 策略（anon 只读 + authenticated 读写）？
- 确认后我将把 00_init.sql 文件写入仓库并提交，然后开始后端 API 的 Supabase 适配与历史数据迁移脚本。