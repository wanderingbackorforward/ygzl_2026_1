        ...
        series: [
            {
                ...
                symbol: 'none',
                areaStyle: {
                    color: 'rgba(0, 229, 255, 0.2)' // Shaded area color
                },
...
function initMainTrendChart() {
    ...
    const initialOption = {
        title: {
            // text: '裂缝变化趋势', // Title managed outside or in update
            left: 'center',
            textStyle: { fontSize: 24, color: '#eee' } // Increased from 18
        },
        tooltip: {
            trigger: 'axis',
            formatter: function(params) {
                if (!params || params.length === 0) return ''; // Handle no data case
                let result = params[0].axisValue + '<br/>';
                params.forEach(param => {
                     if (param.value !== null && param.value !== undefined && !isNaN(param.value)) {
                        result += param.marker + ' ' + param.seriesName + ': ' +
                                 param.value.toFixed(3) + ' mm<br/>'; // Use 3 decimal places
             ...
             textStyle: { fontSize: 16 } // Added/Increased tooltip text size
        },
        legend: {
            type: 'scroll', // Use scrollable legend
            top: 5, // Position legend below controls
            left: 'center',
             width: '70%', // Adjust width as needed
            textStyle: { color: '#eee', fontSize: 18 } // Increased from 14
            // Data will be set in updateMainTrendChart
        },
        grid: {
            left: '5%', // Adjusted grid
            right: '5%',
            bottom: '15%', // Adjusted top spacing for legend
            top: '60px', // Adjusted top spacing for legend
            containLabel: true
        },
        toolbox: {
            feature: {
            ...
            right: '20px',
            top: '5px' // Adjusted toolbox position
        },
        dataZoom: [
            {
                ...
                textStyle: { color: '#eee', fontSize: 16 }, // Increased from 12
                handleStyle: { color: 'rgba(0,229,255,0.9)', borderColor: 'rgba(0,229,255,0.7)' }
            },
            { type: 'inside', start: 0, end: 100 }
        ],
        xAxis: {
            type: 'category', boundaryGap: false, data: [], // Data set in update
            axisLabel: { rotate: 45, color: '#eee', fontSize: 18 } // Increased from 14
        },
        yAxis: {
            type: 'value', name: '裂缝宽度(mm)', scale: true,
            axisLabel: { color: '#eee', fontSize: 18 }, // Increased from 14
            nameTextStyle: { color: '#eee', fontSize: 18 } // Increased from 14
        },
        series: [] // Series data set in update
    };
...
function initSlopeChart() {
    ...
    const option = {
        ...
        grid: {
            ...
            top: '15%', // Add top margin for title and potential labels
            containLabel: true
        },
        ...
        xAxis: {
            ...
            axisLabel: {
                ...
                color: '#eee' // Ensure label color is visible
            },
            axisTick: { // Align ticks with labels
                alignWithLabel: true
            }
        },
        ...
        yAxis: {
            ...
            nameTextStyle: {
                color: '#eee',
                padding: [0, 0, 0, 50], // Adjust padding if name overlaps labels
            ...
            splitLine: { // Add horizontal grid lines for better readability
                lineStyle: {
        ...
        series: [
            {
                ...
                label: {
                    ...
                    color: '#ccc', // Use a contrasting color
                     distance: 5 // Add some distance from the bar top
                },
                itemStyle: {
                    color: function(params) {
                        // Use the pre-calculated sorted colors
                        return sortedColors[params.dataIndex];
                    }
                },
                 // Optional: Add emphasis style on hover
                 emphasis: {
...
function initRateChart() {
    ...
    const option = {
        ...
        grid: { // Adjust grid for bar chart
            left: '3%',
            right: '4%',
            bottom: '10%', // Increase bottom margin if needed
            top: '15%', // Add top margin for title/labels
            containLabel: true
        },
        xAxis: {
        ...
        yAxis: {
            ...
            nameTextStyle: {
                color: '#eee',
                padding: [0, 0, 0, 50], // Adjust padding
                fontSize: 18 // Increased font size
            },
            axisLabel: {
                color: '#eee',
            ...
            splitLine: { // Add horizontal grid lines
                lineStyle: {
                    color: 'rgba(255, 255, 255, 0.1)'
                }
            }
...
function initCorrelationChart() {
    ...
    const option = {
        ...
        xAxis: {
            type: 'category',
            data: pointIds,
            axisLabel: { interval: 0, rotate: 45, fontSize: 16 }, // Increased from 10
            splitArea: { show: true }
        },
        yAxis: {
            type: 'category',
            data: pointIds,
            axisLabel: { interval: 0, fontSize: 16 }, // Increased from 10
            splitArea: { show: true }
        },
        visualMap: {
            min: minValue,
