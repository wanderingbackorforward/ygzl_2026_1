    ...
    if (selector) {
        selector.addEventListener('change', function() {
            ...
            if (pointId) {
                ...
                console.log(`[charts.js] Dispatching setViewpoint event with ID: ${pointId}`);
                const event = new CustomEvent('setViewpoint', { detail: { pointId: pointId } });
                document.dispatchEvent(event);
            } else {
                console.log(`[charts.js] Dispatching setViewpoint event with ID: Default`);
                const event = new CustomEvent('setViewpoint', { detail: { pointId: 'Default' } });
                document.dispatchEvent(event);
            }
        });
        ...
        // Add initial event dispatch for default view after setup
        // (ensure three_viewer is likely ready by now)
        setTimeout(() => {
             console.log(`[charts.js] Dispatching initial setViewpoint event with ID: Default`);
             const event = new CustomEvent('setViewpoint', { detail: { pointId: 'Default' } });
             document.dispatchEvent(event);
        }, 500); // Add a small delay just in case
    }
}
