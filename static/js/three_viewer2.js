// three_viewer.js - Logic for the 3D Model Test/Tool Page
document.addEventListener('DOMContentLoaded', () => {
    ...
    function recordViewpoint() {
        ...
        console.log(`[three_viewer.js] Recorded viewpoint for ${pointId}:`, outputString);
    }

    // --- Initialization ---
    function init() {
        scene = new THREE.Scene();
        scene.background = new THREE.Color(0x101522); // Dark blue background for test page

        const aspect = container.clientWidth / container.clientHeight;
        camera = new THREE.PerspectiveCamera(60, aspect, 0.1, 1000);
        camera.position.set(0, 1, 5);

        renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true }); // No alpha needed if background color is set
        ...
        btnRecord.addEventListener('click', recordViewpoint);
