...
def _claude_settings() -> Tuple[str, str, str, float, int]:
    ...
    try:
        timeout_s = float((os.getenv("CLAUDE_TIMEOUT_SECONDS") or "25").strip())
    except Exception:
        timeout_s = 25.0
    # Safety cap: Vercel 60s limit, need 15s for DB + network overhead
    timeout_s = min(timeout_s, 25.0)
    try:
        ...
        if mode == "agent":
            try:
                ...
                if not answer and agent_error:
                    # Agent failed - return error immediately, NO chat fallback
                    # Chat fallback wastes another 25s and guarantees Vercel 60s timeout
                    print(f"[WARN] Agent failed after {_agent_elapsed:.1f}s: {agent_error}")
                    return jsonify({
                        "status": "error",
