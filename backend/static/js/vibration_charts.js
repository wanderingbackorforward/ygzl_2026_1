...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        if (uploadFileBtn) uploadFileBtn.addEventListener('click', uploadFiles);
        ...
        if (datasetSelector) datasetSelector.addEventListener('change', function() {
        ...
        if (channelSelector) channelSelector.addEventListener('change', function() {
            const channelId = this.value;
            const datasetId = datasetSelector ? datasetSelector.value : '';
            if (channelId && datasetId) {
                loadChannelData(datasetId, channelId);
            }
        });
    } catch (e) {
        console.warn('振动页面初始化部分控件缺失，已跳过绑定:', e);
    }
});
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (datasets && datasets.length > 0) {
                    ...
                    if (selector) selector.value = first.id;
                    loadDataset(first.id);
                }
            } else {
...
function updateVibrationStats(stats) {
    const m = document.getElementById('mean-value');
    const s = document.getElementById('std-value');
    const p = document.getElementById('peak-value');
    const r = document.getElementById('rms-value');
    const c = document.getElementById('center-freq');
    if (m) m.textContent = stats.mean_value.toFixed(4);
    if (s) s.textContent = stats.standard_deviation.toFixed(4);
    if (p) p.textContent = stats.peak_value.toFixed(4);
    if (r) r.textContent = stats.root_mean_square.toFixed(4);
    if (c) c.textContent = stats.center_frequency.toFixed(2) + ' Hz';
}

// 初始化时域图表
function initTimeDomainChart() {
    const el = document.getElementById('channel-time-series') || document.getElementById('time-domain-chart');
    if (!el) return;
    const chart = echarts.init(el, 'cyberpunk');
...
function initFrequencyDomainChart() {
    const el = document.getElementById('frequency-spectrum-chart') || document.getElementById('frequency-domain-chart');
    if (!el) return;
    const chart = echarts.init(el, 'cyberpunk');
