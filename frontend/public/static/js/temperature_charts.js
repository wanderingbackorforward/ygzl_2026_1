...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (selectFileBtn) selectFileBtn.addEventListener('click', function() {
...
function updateTemperatureStats(stats) {
    ...
    // 更新传感器总数 (Previously 数据点数)
    const pointCountElement = document.getElementById('current-point-count');
    if (pointCountElement && stats.current_temperature.sensor_count !== undefined) { // Use sensor_count
        pointCountElement.textContent = stats.current_temperature.sensor_count; 
    } else if (pointCountElement) {
        pointCountElement.textContent = '未知';
    }
    
    // 更新数据时段 (Previously 测量日期)
    const dateElement = document.getElementById('current-temp-date');
    if (dateElement && stats.current_temperature.date_range) { // Use date_range
        dateElement.textContent = stats.current_temperature.date_range;
    } else if (dateElement) {
        dateElement.textContent = '未知';
    }
...
function updateTimeSeriesChart(chartData, isMultiSensor = false) {
    ...
    // Dispose of the existing chart instance before re-initializing or setting new options
    if (charts.seriesChart) {
        try {
            charts.seriesChart.dispose();
        } catch (e) {
            console.error("Error disposing existing series chart:", e);
        }
        charts.seriesChart = null; // Ensure we will re-initialize
    }

    // Always (re-)initialize the chart instance now
    try {
        // Check if the DOM element is valid before initializing
        if (!chartDom || typeof chartDom.getAttribute !== 'function') {
             console.error("Series chart DOM element is invalid before init.");
             showEmptyDataMessage('temperature-series-chart', '图表容器无效');
             return;
        }
        charts.seriesChart = echarts.init(chartDom);
         // Add resize listener *after* successful init
         // Use a flag to prevent adding multiple listeners if called repeatedly without reset
         if (!chartDom._hasResizeListenerForSeries) {
            window.addEventListener('resize', () => {
                // Check if chart instance still exists before resizing
                if(charts.seriesChart && charts.seriesChart.getDom()) {
                    charts.seriesChart.resize();
                }
            });
            chartDom._hasResizeListenerForSeries = true; // Add flag to the DOM element
        }
    } catch (e) {
    ...
    // Prepare chart data (Logic largely unchanged, added refined empty checks)
    let seriesData = [];
    let legendData = [];
    let xAxisData = []; // Use the longest date range for x-axis
    let validData = []; // For single sensor valid points check
    ...
    if (isMultiSensor) {
        ...
        if (sensorIds.length === 0 || Object.values(chartData).every(data => !data || data.length === 0)) {
            ...
            return; // Return after initializing empty chart
        }
            ...
            const avgTemps = xAxisData.map(date => tempMap.get(date) ?? null);
             ...
             if (avgTemps.some(temp => temp !== null)) {
                seriesData.push({
                    ...
                    smooth: true,
                    showSymbol: false,
                    connectNulls: false, // Important: Do not connect nulls
        ...
        validData = timeSeriesData.filter(item =>
            item &&
        ...
        seriesData = [
            {
                name: '平均温度', type: 'line', data: avgTemps, smooth: true,
                lineStyle: { width: 3, color: '#00e5ff' }, itemStyle: { color: '#00e5ff' },
                emphasis: { focus: 'series' }, connectNulls: false
            },
            {
                name: '最低温度', type: 'line', data: minTemps, smooth: true,
                lineStyle: { width: 2, color: '#0088ff' }, itemStyle: { color: '#0088ff' },
                emphasis: { focus: 'series' }, connectNulls: false
            },
            {
                name: '最高温度', type: 'line', data: maxTemps, smooth: true,
                lineStyle: { width: 2, color: '#ff3e5f' }, itemStyle: { color: '#ff3e5f' },
                emphasis: { focus: 'series' }, connectNulls: false
            }
        ];
    }

    // Configure chart option (Unchanged)
    const option = {
        title: {
            text: isMultiSensor ? '多传感器温度对比' : '温度时间序列',
            left: 'center',
            textStyle: { color: '#eee', fontSize: 24 }
        },
        tooltip: {
            trigger: 'axis',
        ...
        legend: {
            data: legendData, textStyle: { color: '#ddd', fontSize: 18 },
            top: 'bottom', type: 'scroll', padding: [20, 10, 5, 10]
        },
        grid: {
            left: '3%', right: '4%', bottom: '25%', containLabel: true
        },
        xAxis: {
            type: 'category', boundaryGap: false, data: xAxisData,
            axisLabel: { color: '#ddd', rotate: 45, hideOverlap: true, fontSize: 18 }
        },
        yAxis: {
            type: 'value', name: '温度 (°C)', nameTextStyle: { color: '#ddd', fontSize: 18 }, axisLabel: { color: '#ddd', formatter: '{value}°C', fontSize: 18 }, scale: true
        },
        dataZoom: [
             { type: 'inside', start: 0, end: 100, filterMode: 'filter' },
             { type: 'slider', start: 0, end: 100, filterMode: 'filter',
               ...
               bottom: '5%', height: 20, borderColor: '#444', backgroundColor: 'rgba(255,255,255,0.1)',
               textStyle: { fontSize: 16 } },
        ],
        series: seriesData
    };
    ...
    try {
        // Set the option on the newly initialized chart
        charts.seriesChart.setOption(option, true); // Use true to not merge
    } catch (e) {
        console.error("Failed to set option on series chart:", e);
        showEmptyDataMessage('temperature-series-chart', '更新图表失败');
    }

     // Resize listener setup moved after successful init
     // Ensure container is visible - handled by height reset earlier
}
...
function updateTemperatureRangeChart(timeSeriesData) {
    ...
    if (emptyDiv) {
     ...
     // Reset height
    if (!chartDom.style.height || chartDom.style.height === '0px') {
         chartDom.style.height = '400px'; // Default chart height
    }

    // Dispose of the existing chart instance
    if (charts.rangeChart) {
         try {
            charts.rangeChart.dispose();
         } catch (e) {
             console.error("Error disposing existing range chart:", e);
         }
        charts.rangeChart = null; // Ensure we re-initialize
    }

     // Check if data is empty *before* initializing
    if (!timeSeriesData || timeSeriesData.length === 0) {
        showEmptyDataMessage('temperature-range-chart', '暂无该传感器的温度范围数据');
        return; // Don't initialize if no data
    }

     // Filter data to ensure range exists
     const validRangeData = timeSeriesData.filter(item => item && item.measurement_date && item.temperature_range !== null && item.temperature_range !== undefined);

     if (validRangeData.length === 0) {
         showEmptyDataMessage('temperature-range-chart', '该传感器无有效日温差数据');
         return; // Don't initialize if no valid data
     }
    ...
    try {
         if (!chartDom || typeof chartDom.getAttribute !== 'function') {
             console.error("Range chart DOM element is invalid before init.");
             showEmptyDataMessage('temperature-range-chart', '图表容器无效');
             return;
         }
        charts.rangeChart = echarts.init(chartDom);
         // Add resize listener *after* successful init
         // Use a flag on the DOM element to prevent multiple listeners
         if (!chartDom._hasResizeListenerForRange) {
             window.addEventListener('resize', () => {
                 if(charts.rangeChart && charts.rangeChart.getDom()) {
                     charts.rangeChart.resize();
                 }
             });
             chartDom._hasResizeListenerForRange = true;
         }
    } catch (e) {
        console.error("Failed to initialize range chart:", e);
        showEmptyDataMessage('temperature-range-chart', '图表初始化失败');
        return; // Stop if init fails
    }


    // Prepare data using filtered data
    const dates = validRangeData.map(item => item.measurement_date);
    const tempRanges = validRangeData.map(item => item.temperature_range);

    // Configure chart option (Unchanged)
    const option = {
        title: {
            text: '日温差变化', left: 'center', textStyle: { color: '#eee', fontSize: 24 }
        },
        tooltip: {
            trigger: 'axis', formatter: function(params) { return params[0].axisValue + '<br/>' + params[0].marker + ' 日温差: ' + params[0].value + '°C'; },
            textStyle: { fontSize: 16 }
        },
        grid: { left: '3%', right: '4%', bottom: '25%', containLabel: true },
        xAxis: {
            type: 'category', boundaryGap: false, data: dates, axisLabel: { color: '#ddd', rotate: 45, hideOverlap: true, fontSize: 18 }
        },
        yAxis: {
            type: 'value', name: '温度范围 (°C)', nameTextStyle: { color: '#ddd', fontSize: 18 }, axisLabel: { color: '#ddd', fontSize: 18 }
        },
        series: [
            {
                name: '日温差', type: 'bar', data: tempRanges,
                itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [ { offset: 0, color: '#ff3e5f' }, { offset: 1, color: '#0088ff' } ]) },
                emphasis: { itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [ { offset: 0, color: '#ff5f7e' }, { offset: 1, color: '#40a8ff' } ]) } }
            }
        ]
    };
    ...
    try {
        ...
        charts.rangeChart.setOption(option, true); // Use true to not merge
         ...
         console.error("Failed to set option on range chart:", e);
         showEmptyDataMessage('temperature-range-chart', '更新图表失败');
    }
...
function resetSensorCharts() {
    ...
    // Dispose and clear time series chart
    if (charts.seriesChart) {
        try {
            // Remove resize listener associated with the specific DOM element if flag exists
            const seriesChartDom = document.getElementById('temperature-series-chart');
            if (seriesChartDom && seriesChartDom._hasResizeListenerForSeries) {
                 // Note: Removing specific listener is complex, simple dispose should handle it
                 // Or manage listeners globally with chart instances
                 seriesChartDom._hasResizeListenerForSeries = false; // Reset flag
            }
            charts.seriesChart.dispose();
        } catch (e) {
             console.error("Error disposing series chart during reset:", e);
        }
        charts.seriesChart = null;
    }
    showEmptyDataMessage('temperature-series-chart', '请选择传感器查看时间序列数据');

    // Dispose and clear range chart
    if (charts.rangeChart) {
         try {
             const rangeChartDom = document.getElementById('temperature-range-chart');
             if (rangeChartDom && rangeChartDom._hasResizeListenerForRange) {
                 rangeChartDom._hasResizeListenerForRange = false; // Reset flag
             }
            charts.rangeChart.dispose();
         } catch (e) {
             console.error("Error disposing range chart during reset:", e);
         }
        charts.rangeChart = null;
    }
    showEmptyDataMessage('temperature-range-chart', '请选择传感器查看温度范围变化');
    ...
    if (selector) {
        ...
        // Set the selector's value to the placeholder if it exists
         if (selector.options[0] && selector.options[0].value === "") {
             selector.value = "";
         } else {
             // If no placeholder, set to empty (might show first option if not disabled)
             selector.value = "";
         }
    }
}
